...
- [ ] ★ 右栏切到**「工作流架构」**：分六层画出 `langgraph-main.mmd` 的设计；
      提问后走过的节点**依次点亮**、走过的连线会流动
- [ ] ★ 架构栏顶部的**「本次经过」摘要**要说清三件事：
...
- [ ] `node scripts/check_ui.cjs` 全过（61 项）—— 尤其"前端节点表与后端 `add_node` 双向一致"
      （这条守的是跨语言漂移：图里加了新节点、前端没跟上，界面上会冒出"（未登记的节点）"）
      以及 E 组"清库 SQL 必须每条都带 `WHERE is_test = true`"
      （这条守的是**不可逆删除**：子句一旦丢失，代码照跑、测试不报错，
      只会在某次执行时把真实顾客的工单删光）
      以及 F 组"两个页面在桩环境里真跑一遍脚本"、H 组"工作流架构真的画得出来"、I 组"提示条不能说谎也不能卡住"
...
第 25、26 条都是"看起来能用"的类型：手动刷新一下就正常，所以能活很久。
两条的共同修法是**把判断依据收敛到唯一来源**：25 是让两个接口的字段集对齐并有断言盯着，
26 是让提示条只认服务端状态、且每次轮询都同步。
