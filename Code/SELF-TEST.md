...
- [ ] ★ 右栏切到**「工作流图」**：提问后走过的节点**依次点亮**、走过的边有流动效果；
      鼠标悬停能看到中文名与这一步的耗时
      > 图上的连线是从**编译好的图**导出的，不是画着好看的示意图。
      > 条件边的分支目标只有代码里声明过才会画出来，所以界面写的是
      > "实际执行路径以轨迹为准" —— 图上高亮的那条路才是可信的部分。
