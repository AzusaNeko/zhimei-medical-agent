...
| **运营后台（坐席工作台）** | ✅ **已实现并验证**（`scripts/smoke_ops.py` 51 项；面板 + 队列 + 跨模块联动） |
...
> 模型会不会把"会不会失明"误判成紧急等）。接线已经用 fake 档位验过 135 项断言
> （`smoke.py` 96 + `smoke_api.py` 39），清单验的是行为与质量。
...
node scripts/check_ui.cjs                # 前端静态校验（语法 / 节点表同步 / DOM id / 测试工单隔离 / 页面启动 / 工作流架构 / 接管状态，61 项）
```
...
- 右栏可以切换 **执行轨迹 / 工作流图**：图上按真实执行顺序点亮走过的节点
  （见「工作流执行图」一节）
...
### 工作流架构（右栏第二栏，与执行轨迹分开展示）

右栏可以在「执行轨迹」和「工作流架构」之间切换 —— 两者是**分开的两块**：
轨迹回答"这一步做了什么、输出了什么"，架构回答"这次走过了哪条路、进了哪几个子图"。
...
python scripts/smoke_ops.py        # 51 项（不变量、跨模块联动、权限与脱敏、接管）
```
...
│  ├─ check_ui.cjs             # 前端静态校验：语法 / 节点表同步 / 测试工单隔离 / 页面启动 / 工作流架构（61 项）
│  └─ seed_kb.py               # 演示数据播种
