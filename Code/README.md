...
node scripts/check_ui.cjs                # 前端静态校验（语法 / 节点表同步 / DOM id / 测试工单隔离 / 页面启动，35 项）
```
...
| 接管状态（`ai_enabled`） | 随上面两条一起同步 | 顾客端据此**禁用输入框**，而不是让他发出去再收 409 |
...
│  ├─ check_ui.cjs             # 前端静态校验：语法 / 节点表同步 / 测试工单隔离 / 页面真实启动（35 项）
│  └─ seed_kb.py               # 演示数据播种
