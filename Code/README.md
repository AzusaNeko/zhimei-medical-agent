...
> 模型会不会把"会不会失明"误判成紧急等）。接线已经用 fake 档位验过 146 项断言
> （`smoke.py` 96 + `smoke_api.py` 50），清单验的是行为与质量。
...
python scripts/smoke_api.py                              # 接口层冒烟（50 项，进程内跑 ASGI）
```
...
- 每条历史对话右侧有**删除按钮**（鼠标移到那一行才显出来）。删除前会逐条说明
  "删什么、留什么" —— 见「删除历史对话」一节
- 会话被坐席接管后，会显示"人工客服处理中：AI 已暂停自动回复，你发的消息会直接转给客服"，
  但**输入框不禁用** —— 你仍然可以补充情况，客服看得到。
- 右栏**同屏**显示两块：上面是**工作流架构**（本次走过了哪条路、进了哪几个子图），
  下面是**执行轨迹**（每一步做了什么）。两个窗口各自独立滚动
  （见「工作流架构」一节）
- 正文是**打字机**呈现的：内容仍然整段审后送达，动画只是送达之后的显示
  （点一下可以立刻看全文）
...
做的是**真删除**，不是"从列表里藏起来"。一次清四样东西：

| 删什么 | 在哪 | 为什么必须清 |
|---|---|---|
| 全部消息 | `app.chat_message` | 对话本身 |
| 会话行 | `app.chat_session` | 列表、channel、`ai_enabled` 都在这 |
| 工单里的对话快照与草稿 | `ops.handoff_ticket.last_turns` / `risk_report.draft` | **最容易漏**：工单里存着最近 5 轮的原文快照。不清它，"删了"其实没删 |
| LangGraph 的图状态 | `lg.*`（checkpointer） | **第二容易漏**：它在另一个 schema 里、由 saver 自己建表，业务代码里一行都看不到。整份图状态（草稿、审查意见、槽位）都在里面 |

> ★ 这两处"最容易漏"是这类功能翻车的地方：只删业务表也能让界面看起来"删干净了"。
> 所以冒烟里专门有一条断言去问 checkpointer **这个 thread 还在不在**
> （`_thread_exists`），而不是只看业务表。

**保护条件**：会话还有**未结束**的工单时拒绝删除（409 `session_has_open_ticket`）。
人工正在处理这件事，把会话抽走会让坐席对着一个空详情页发呆，而他手上没有任何线索
能解释发生了什么。用户会被告知"等客服处理完再删"。

**故意保留**：`app.review_audit`（裁决结论与规则命中）与工单行本身。
医美系统里"顾客说过什么"可以删，"系统当时为什么放行/拦截"不能删 ——
保留的动作会写进工单的 `risk_report.transcript_deleted`，
坐席打开详情时会看到一条黄色说明，**不会**误以为系统坏了。

响应体里如实报告删了什么、留了什么（`removed` / `kept`），前端确认框也逐条列出 ——
不假装这是一次彻底的抹除。



### 用户主动要求转人工：先安抚，连续三次才真的转

用户随时可以说"转人工"。这条链路有三个刻意的设计：

| 第几次 | 行为 | 为什么 |
|---|---|---|
| 第 1–2 次 | **安抚回复**（固定模板），说明"当前是第 N 次 / 连续 3 次就会转"，并趁这时间把已说过的情况整理成摘要 | 中文里"人工"两个字太随意（"人工客服几点下班""人工费怎么算"），一次就转会让转人工率虚高、坐席被无谓占用 —— 与"二次复核白升"是同一类问题 |
| 第 3 次 | 建工单 + **当场停用 AI** + 转接提示 | 用户明确要人，再让 AI 抢话就是不尊重他的选择 |
| 已接管后再说 | 安抚回执（"已经有同事在处理了，这是第 N 次，已记录"），不重复建单 | 已经转过了，再建一张只会让坐席看到两张重复工单 |

四个细节值得单独说：

- **门槛必须如实告诉用户。** 含糊地说"稍后为您转接"再拖三次，那是在骗人。
  所以话术里直接写明"连续说 3 次「转人工」我就直接为您转接"，并报出当前是第几次。
- **话术用固定模板，不走 LLM。** 这几句是**关于流程的承诺**，
  让模型现场措辞，它可能随口承诺一个我们并不执行的时限或次数。
- **紧急信号优先。** "我脸发白还特别疼，麻烦转人工"不能被这条拦截 ——
  先判紧急，是紧急就照旧走图（急诊模板 + 转人工并行）。
  宁可走完整流程，也不能因为一句话里带"人工"就把急诊降级成排队。
- **计数放在数据库**（`chat_session.human_request_count`），不是图状态：
  接管期间**根本不跑图**，而用户恰恰是在那个阶段最容易反复要求转人工。
  自增用一条 `UPDATE ... RETURNING`，不是"读出来 +1 再写回"（并发会丢计数，
  而丢计数意味着用户说了三次却一直不转）。

> ⚠️ 识别"要求转人工"用的是关键词 + 排除词，不是模型判断：这件事的结果是
> **把会话转给真人**，代价高、必须可预测。而且必须处理问句 ——
> 第一版只收了关键词，"人工客服几点下班"当场被误判成转人工请求。
> 与紧急词表处理"会不会…"是同一个取向：**字面匹配必须处理问句**。

### 工作流架构（与执行轨迹同屏，但各自独立成窗）

右栏**上下两块同时显示**，取消了一栏切换：

```text
┌─ 工作流架构 ── 本次 27 个节点 · 2 个子图 ───────────┐   ← 固定高度、独立滚动
│  本次对话经过： [✓入口与调度] [✓草稿产出(p_agent)] …  │
│  [✓知识科普子图（8 步：…）] [✓风险审查子图（16 步）]   │
│  ┌─── 分层架构图（走过的节点点亮、连线流动）───┐      │
└──────────────────────────────────────────────────┘
┌─ 执行轨迹 ───────────────────────────────────────┐   ← 吃掉剩余空间、独立滚动
│  1 输入标准化与授权校验 normalize   41ms            │
│  2 意图识别与槽位抽取 classify     1280ms  LLM      │
└──────────────────────────────────────────────────┘
```

**为什么同屏**：它们回答的是两个不同的问题，而且看的时候往往是**同时**看 ——
"这一步怎么走的"（轨迹）和"整条路走到哪了"（架构）。做成切换的话，
每次对照都要点一下、还得记住刚才那一屏，等于把一份信息拆成两半让人自己拼。

**为什么仍要分成两个窗口**：各自有自己的标题栏与滚动条，互不干扰 ——
滚架构不会把轨迹滚跑，反之亦然。

架构照 **`Workflow/langgraph-main.mmd`** 那张设计图画，分六层：

```text
入口与调度  →  七路草稿产出  →  聚合与必经关卡  →  修订回路  →  输出与执行  →  出口层
```

**"本次经过"摘要**（架构窗口顶部常驻）直接回答"这一次经过了哪些子图"：

- 每一层**这次走过没有**、走过几个节点；
- 草稿产出层具体是哪几个 Agent 跑的（例如「草稿产出层（术后护理）」）；
- **哪几个子图被进入过**、各走了几步（例如「✓ 知识科普子图（8 步：入口分流、上下文装配…）」）；
- 执行节点总数；以及架构与代码**是否一致**。

设计图是手写的，所以这里必须防它过期。做法是**交叉校验**（`graph_topology.validate`）：

| 情况 | 含义 | 处理 |
|---|---|---|
| 架构里有、代码里没有 | 设计图过期了 | 报 warning 并显示在界面上 |
| 代码里有、架构没登记 | 加了节点没更新设计图 | 同上，并在 `smoke.py` 第 0 节断言失败 |

> 这道校验第一次跑就抓到两个"代码里有、设计图没画"的节点
> （`set_result_kind`、`need_info`）—— 不是我想象出来的问题，是真发生的漂移。
> **没有这层校验，这张图迟早变成"看着很专业但和代码不符"的装饰品，
> 而画错的架构图比没有图更糟。**

另一个设计点：**菱形 = 路由点**（对应 `.mmd` 里的 `{}` 节点），连线上的标签就是
路由函数的分支含义（`pass 通过` / `revise 需修改` / `human 高风险`…）。
黄色小点表示这一步会调用大模型。

### 关于"流式输出"

做的是**逐字呈现**，不是真的流式发送正文 —— 这是刻意的：

```text
内容必须整段通过风险闸（硬性规则 + 三份面板意见）之后才能出站。
如果按 token 往外吐，用户看到的就是**未经审查的内容**：
违规用语、未核实的疗效承诺、还没过隐私检查的个人信息，都会先到他眼前。
那等于把整个风控前提拆掉。

另外这里还有个具体障碍：专业 Agent 的产出是 JSON（{content, citations…}），
按 token 流出来，用户会先看到一屏 JSON 碎片。
真流式必须新增一条"直接生成散文"的链路，而那条链路的内容
在生成完之前没经过任何审查。
```

所以 `final` 事件仍然是**一个完整事件**，前端在它到达之后逐字显示。
一个字的顺序都没有提前。三个细节：总时长固定（不随文本长度线性增长）、
**点一下立刻显示全文**（长医疗文本没人愿意干等）、尊重 `prefers-reduced-motion`。

> ★ 角标写的是「**逐字呈现 · 整段过审后送达**」。
> 之前写的是"整段送达（不流式）"—— 那句话描述的是**传输方式**，
> 而用户看到的是**呈现方式**：一边逐字往外走、一边标着"不流式"，
> 只会让人以为坏了。角标该说的是"这份正文是整段过审之后才发出来的"，
> 那才是它想表达的保证。

这几条都锁在 `check_ui.cjs` 的 G 组里，包括"后端不许出现分片事件"
和"角标不许再写不流式"。

| 端点 | 作用 |
|---|---|
| `GET /api/sessions` | 会话列表（标题 / 消息数 / 最近活跃 / ai_enabled）。**按当前登录用户过滤**，支持 `channel` 与 `limit` |
| `POST /api/sessions` | 新建会话 |
| `GET /api/sessions/{sid}` | 单个会话的消息记录 |
| `GET /api/graph` | 工作流架构 + 真实图 + 两者的不一致清单 |

> ⚠️ `/chat` 与 `/ops/panel` 都带 `Cache-Control: no-store`。
> 这两个单文件页面是**边改边用**的，没有这个头浏览器会按启发式规则缓存旧版本 ——
> 于是"改了前端、刷新还是老行为"，排查方向会被带偏。
...
`GET /ops/tickets` **默认只返回待处理的**（`open` / `accepted` / `in_progress` / `escalated`），
要看历史得显式传 `?status=closed`。这个默认值放在**服务层**（`ops.service.ACTIVE_STATUSES`）
而不是路由层，所以 REST 与 SSE 快照自动同一口径 —— 不会出现"接口过滤了、推送没过滤"。

> 为什么必须过滤：已结束的工单留在队列里，坐席每次打开面板都要重扫一遍历史，
> 真正待接单的那张被埋在下面；更糟的是它的 `msg_count` 还会变，
> 让面板误以为"这个顾客又说话了"，在工单结束之后继续弹提示 ——
> 这就是"工单都结束了还能收到顾客请求"的来源。

**工单结束 = 人工退场。** 关单后 AI 恢复自动回复，顾客再说话是找 AI 的，
不该再推到坐席面前。前端 `maybeRefreshDetail()` 里因此有一条判断：
行状态不在待处理集合内就**停止跟踪**，只留一句"该工单已结束，不再跟踪新消息"。

队列每一行显示三件事，都是为了"扫一眼就知道先接哪个"：

| 显示 | 来源 | 说明 |
|---|---|---|
| **发起人**（脱敏） | `app_user.display_name` | 只留**第一个字**，其余打码：`演***`。队列是一屏几十行的列表，只需区分"是不是同一个人的事"，全名在这里没有增量信息、却把一批顾客的姓名集中暴露在一个页面上 |
| **已等待时长 / SLA 基准** | `wait_seconds` / `sla_seconds` | 超时会变红加粗（`sla_breached`） |
| 一行上下文 | `profile_summary` 首行 + 转人工原因 | 否则坐席得点进去才知道要不要先接 |

看全名要点进**详情**（那里按角色权限决定：`doctor`/`compliance`/`admin` 见明文，
`service` 见脱敏）—— 一次只看一个人，和"一屏几十行"的风险完全不同。
