...
| 项 | 状态 |
|---|---|
| 图的接线、路由、预算、凭据、并发写冲突 | ✅ **已跑通**（`scripts/smoke.py` 91 项断言全过） |
...
python scripts/smoke.py                # 91 项不变量断言
...
> 模型会不会把"会不会失明"误判成紧急等）。接线已经用 fake 档位验过 130 项断言
> （`smoke.py` 91 + `smoke_api.py` 39），清单验的是行为与质量。
...
node scripts/check_ui.cjs                # 前端静态校验（语法 / 节点表同步 / DOM id / 测试工单隔离 / 页面启动 / 工作流图，51 项）
```
...
右栏可以在「执行轨迹」和「工作流图」之间切换。图上的节点与连线是**从编译好的图上
直接导出**的（`GET /api/graph` → `app/api/graph_topology.py`），不是前端写死的：

> 前端那份「节点 → 中文名」表已经是手写的、也已经漂移过一次（靠 `check_ui.cjs`
> 的 B 组静态比对兜住）。再手写一份"连线和布局"进去，就是第二处必然漂移的副本 ——
> 而**一张少了边的流程图比没有图更糟**，它会让人对执行过程产生错误判断。

三个设计点：

- **布局是算出来的**（分层 DAG：层号 = 从入口算起的最长路径），不是手填坐标。
  节点有 55 个，手填坐标意味着"图里加一个节点就得回来改坐标"，又是一处必然漂移的地方。
  它不保证好看，但保证**每次都能画出来**，包括以后新增的节点。
- **子图要单独导出并显式连上挂载点**（`k_agent → kb_intake`、`risk_gate → gate_in`）。
  子图在主图里是**黑盒节点**，不补这几条边，两个子图在图上就是孤岛。
- **如实标注一处局限**：条件边的分支目标只有在代码里声明过
  （`add_conditional_edges(..., {..})` 或 `add_node(..., destinations=)`）才会出现在导出里。
  没声明的分支在图上会显示成"走到头了"。所以界面上的说法是
  "实际执行路径以轨迹为准" —— 图上会按真实执行顺序实时高亮，那才是可信的部分。

实际执行时：走过的节点**依次点亮**（主图深蓝 / 科普子图青色 / 风险子图红色），
走过的边变成**流动的虚线**（这就是"数据流向"的可视化），节点上标出这一步的耗时，
鼠标悬停有中文名与说明。

### 关于"流式输出"

做的是**打字机呈现**，不是真的流式发送正文 —— 这是刻意的，不是偷懒：
...
一个字的顺序都没有提前，内容安全性完全不变。三个细节：总时长固定（不随文本长度
线性增长）、**点一下立刻显示全文**（长医疗文本没人愿意干等）、
尊重 `prefers-reduced-motion`。这三条都锁在 `check_ui.cjs` 的 G 组里，
顺带锁住"后端不许出现分片事件"。
...
│  ├─ check_ui.cjs             # 前端静态校验：语法 / 节点表同步 / 测试工单隔离 / 页面启动 / 工作流图（51 项）
│  └─ seed_kb.py               # 演示数据播种
